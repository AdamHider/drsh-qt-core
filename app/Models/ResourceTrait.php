...
trait ResourceTrait
{
    ...
    public function recalculateResources ($data) 
    {
        ...
        foreach($data as $code => &$value){
            ...
            $this->USR_MOD = $this->getResourceModifier($code);
    ...
    private function recalculateValue ($value)
    {
    ...
    private function getResourceModifier ($code)
    {
        if(!session()->get('user_id')){
            return 1;
        }
        $SettingsModel = model('SettingsModel');
        $setting = $SettingsModel->join('settings_usermap', 'settings_usermap.item_id = settings.id AND settings_usermap.user_id = '.session()->get('user_id'))
        ->select('settings_usermap.value')->where('settings.code', $code.'GainModifier')->get()->getRowArray();
